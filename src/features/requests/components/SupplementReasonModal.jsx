      ...
      >
        <div className="flex justify-between items-start p-5 border-b border-outline-variant/30">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="font-headline-sm text-headline-sm text-on-surface">Yêu cầu bổ sung</h2>
              <p className="text-xs text-secondary mt-0.5">Đơn {requestId}</p>
            </div>
          </div>
        ...
        <div className="p-5">
          <label className="block font-label-md text-label-md text-on-surface-variant mb-1.5">Lý do / Thông tin cần bổ sung</label>
          <textarea
            autoFocus
            value={reason}
            ...
            placeholder="Nhập yêu cầu bổ sung thông tin cụ thể (bắt buộc)..."
          />
          {!reason.trim() && (
            <p className="text-xs text-on-surface-variant mt-2 flex items-center gap-1">
              <span className="material-symbols-outlined text-[14px]">info</span>
              Vui lòng nhập thông tin để gửi yêu cầu.
            </p>
          )}
        </div>
